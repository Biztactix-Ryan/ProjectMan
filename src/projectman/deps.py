...
def validate_dependencies(
    ...
    """Validate proposed depends_on list, returning error strings.

    Checks for:
    - Self-references
    - Non-sibling references (deps outside the story)
    - Would-create-cycle scenarios
    """
    errors: list[str] = []
    ...
    for dep in depends_on:
        ...
        elif dep not in sibling_ids:
            errors.append(
                f"Dependency {dep} is not a sibling task in story {story_id}"
            )

    if errors:
        return errors

    # Check for cycles: build a graph with the proposed change applied
    graph = build_dep_graph(siblings)
    graph[task_id] = [dep for dep in depends_on if dep in sibling_ids]

    cycle = detect_cycle(graph)
    if cycle is not None:
        path = " -> ".join(cycle)
        errors.append(f"Adding dependencies would create a cycle: {path}")
