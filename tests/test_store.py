...
class TestStoreStories:
    ...
    def test_get_story(self, store):
        ...
        meta, body = store.get_story("TST-1")
        assert meta.title == "My Story"
...
class TestStoreTasks:
    def test_create_task(self, store):
        ...
        task = store.create_task("TST-1", "Task 1", "Do something")
        assert task.id == "TST-1-1"
        assert task.story_id == "TST-1"
    ...
    def test_get_task(self, store):
        store.create_story("Story", "Desc")
        store.create_task("TST-1", "Task 1", "Task body")
        meta, body = store.get_task("TST-1-1")
    ...
    def test_list_tasks_by_story(self, store):
        ...
        store.create_task("TST-1", "Task A", "Desc")
        store.create_task("TST-1", "Task B", "Desc")
        store.create_task("TST-2", "Task C", "Desc")

        tasks = store.list_tasks(story_id="TST-1")
        assert len(tasks) == 2
        tasks = store.list_tasks(story_id="TST-2")
        assert len(tasks) == 1
...
class TestStoreUpdate:
    def test_update_story(self, store):
        store.create_story("Story", "Desc")
        updated = store.update("TST-1", status="active")
        assert updated.status.value == "active"

    def test_update_task(self, store):
        store.create_story("Story", "Desc")
        store.create_task("TST-1", "Task", "Desc")
        updated = store.update("TST-1-1", status="in-progress", assignee="alice")
    ...
    def test_archive_story(self, store):
        store.create_story("Story", "Desc")
        store.archive("TST-1")
        meta, _ = store.get_story("TST-1")
        assert meta.status.value == "archived"

    def test_archive_task(self, store):
        store.create_story("Story", "Desc")
        store.create_task("TST-1", "Task", "Desc")
        store.archive("TST-1-1")
        meta, _ = store.get_task("TST-1-1")
