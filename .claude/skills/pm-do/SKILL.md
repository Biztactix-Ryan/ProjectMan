...
8. Verify ALL DoD criteria are met — check off each item
   - **If blocked or failing:** Call `pm_update(task_id, outcome="blocked|failed|partial", note="<what happened and what's needed>")` before stopping. This logs the attempt so the next session has context.

## Phase 3: Complete

9. **If `--complete` flag is set:**
   - Call `pm_update(task_id, status="done", outcome="success", note="<brief summary of what was done>")` — do NOT offer a review option
   ...
   **Otherwise (default):**
   - Call `pm_update(task_id, status="review", outcome="success", note="<brief summary>")` if the task needs human review,
     OR call `pm_update(task_id, status="done", outcome="success", note="<brief summary>")` if all DoD criteria are met
