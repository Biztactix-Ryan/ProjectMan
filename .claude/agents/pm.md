...
- Use `pm_batch_get(type)` when you need all items of a type (epics, stories, or tasks) — much faster than individual `pm_get` calls
- Use `pm_get` for single item lookups
